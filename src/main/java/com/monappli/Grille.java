...
public class Grille {
  ...
  public GridPane initGrid() throws ClassNotFoundException, IOException {
    ...
    for (Ilot ilot : listeIlot) {
      ...
      // IleAct is the current active isle and if there is already one, we want to
      // make a bridge between isleAct and ilot
  ...
  public void chargerSauvegarde() throws ClassNotFoundException, IOException {
    ...
    if (save_file.exists()) {
      ...
      /**
       * On itère sur un ilot de la listeIlot et on vérifie ses voisins pour former un
       * pont
       */
        ...
        for (Pont pont : sauvegarde.getPileCoups()) {
          for (Ilot ilot2 : this.listeIlot) {
            if (this.sontVoisin(ilot2, ilot) && !ilot.equals(ilot2)) {
              ...
              /**
               * En comparant les 2 ilots voisins aux ilots composant un pont depuis la
               * sauvegarde, on affiche le pont et on met le nombre de trait du pont de
               * listeIlot à celui du pont de la sauvegarde
               */
   ...
   * @param il1
   * @param il2
   * @return <code>true</code> if the two isle are neighbours, <code>false</code>
   *         if not
   * @author Ambre Collard
   */
  ...
  /**
   * For all bridges, verify if the bridge in parameters doesn't cross any of the
   * already existing bridges
   * 
   * @param pont Bridge to verify
   * @return <code>true</code> if the bridge crosses another one,
   ...
   * 
   * @return <code>true</code> if the grid is completed, <code>false</code>
   *         otherwise.
   */
  ...
  /**
   * Reset the ilses' border to their active border state if they are currently
   * red bordered.
   * 
   * @see Ilot#setRed
   */
