...
public class Ilot implements Comparable<Ilot> {
	...
	private class Btn extends Button {
		...
		public void setStyleParam(int longueur, int largeur) {
			this.setStyle("-fx-background-radius: 200px;" +
					...
					"\n-fx-font:"
					+ (int) (1.0 * 24 / Math.pow(
							(largeur > longueur ? largeur : longueur) / (largeur > longueur ? longueur : largeur),
							1.0 / 4))
					+ " px;");
		}
	...
	public void setOnAction() {
		this.getBtn().setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent e) {
				this.getBtn().setActive(!this.getBtn().getActive());
			}
		});
	}
	...
	/**
	 * Détermine si l'ilot courant et un autre ilot sont égaux en prenant en compte
	 * uniquement leurs coordonnées x et y
	 *
	 * @param obj ilot à comparer
	 * @return true si les coordonnées x et y de l'objet courant et ilot sont égales
	 * @throws ClassCastException si une instance d'une autre classe qu'Ilot est
	 *                            fourni en paramètre
	 */
	@Override
